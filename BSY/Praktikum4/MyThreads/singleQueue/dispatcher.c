...
void dispatchInit(int slice) {
    ...
    if (dispatcherState == 0) {
        ...
        rv = sigaction(SIGINT, &sigAction, NULL);
        if (rv < 0) perror("sigaction failed: ");

        sigAction.sa_handler = preemptionHandler;       // signal handler timer
        rv = sigaction(SIGVTALRM, &sigAction, NULL);
        if (rv < 0) perror("sigaction failed: ");
...
void dispatchTask(int argument)
{
    ...
    switch (schedArg)    {
	    ...
	    SAVE_FX(mtGetFXMemoryPointer(activeThread));    // thread save state of FPU etc...
    ...
    if  ((shutdown) || (activeThread == NULL)) {
	    ...
	    RESTORE_FX();			    // thread:    restore state of FPU etc...
            startTimeSlice();                       // start time slice
        }
    }
...
void finishTimeSlice(void)  {
    // block signal
    sigset_t sigSet;
    if (sigemptyset(&sigSet) < 0)                   perror("signal error: ");
    if (sigaddset(&sigSet, SIGVTALRM) < 0)          perror("signal error: ");
    if (sigprocmask(SIG_BLOCK, &sigSet, NULL) < 0)  perror("signal error: ");
}
...
void startTimeSlice(void) {
    ...
    if (sigemptyset(&sigSet) < 0)                     perror("signal error: ");
    if (sigaddset(&sigSet, SIGVTALRM) < 0)            perror("signal error: ");
    if (sigprocmask(SIG_UNBLOCK, &sigSet, NULL) < 0)  perror("signal error: ");
