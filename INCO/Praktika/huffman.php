...
while (!feof($handle)) {
	...
	// space char ist erstes ASCII Symbol
	if ($x >= ' ') {
		$length++;
		// Build statistics:
		if (!isset($counts[$x])) {
...
foreach ($counts as $symbol => $count) {
	...
	// Initialize an empty code and length zero for each symbol.
...
while (count($tree) > 1) {
	...
	// - Merge the two branches with smallest count in tree.
	...
	reset($tree);
	$tree[$i1 . $i2] = $length1 + $length2;
	
	foreach ($codes as $symbol => $n) {
		if (strpos($i1, $symbol) !== false) {
			$codes[$symbol]["code"] = "1" . $codes[$symbol]["code"]; 
			$codes[$symbol]["length"]++;
		}
		if (strpos($i2, $symbol) !== false) {
			$codes[$symbol]["code"] = "0" . $codes[$symbol]["code"]; 
			$codes[$symbol]["length"]++;
		}
	}
...
foreach($codes as $symbol => $n) {
	...
	$avgCodeLength += $probability[$symbol] * $n["length"];
}
printf("Total %d\n\n", count($codes));
printf("Mittlere Codewortlänge : " . $avgCodeLength . "\n\n");
