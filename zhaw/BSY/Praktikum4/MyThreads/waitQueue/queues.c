...
mthread_t* mqGetNextThread(void) {
    mthread_t *waitingTCB = mlDequeue(waitQueue);
	...
	waitingTCB = mlDequeue(waitQueue);
    }

    for (int i = HIGH; i < NUM_PRIO_QUEUES; i++)
        if (mlGetNumNodes(readyQueues[i]) > 0)
            return mlDequeue(readyQueues[i]); 
...
void mqAddToQueue(mthread_t *tcb, int where) {
	...
	(void)printf("go to sleep for %d ms\n", where);
    }
    else
    {
        mlEnqueue(readyQueues[tcb->tPrio], tcb);
	(void)printf("added to ready queue, prio=%d\n", tcb->tPrio);
    }
}
